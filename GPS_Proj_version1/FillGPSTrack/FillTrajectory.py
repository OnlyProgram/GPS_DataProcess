...
def FilterRoute(filepath, minKilometer=0.5, maxKilometer=1, timechoose=1, Calculation=1):
    ...
    if (Calculation == 1):
        if (timechoose == 1):
            try:
                df = pd.read_csv(filepath, header=None, usecols=[1, 2, 3, 4, 5])
...
#起始想法  有点low
def FillTrack(Trunkpath, trunkdict,savepath,savename, startend,choose=0):
    """
    轨迹填充函数，参数choose只能为0,1.代表两种补点策略
    :param Trunkpath 存储车辆的文件夹路径
    :param trunkdict 经过此路段的车号字典
    :param choose: 默认=0，采用一辆车(此路段点最多)轨迹补点，=1采用此路段所有有效车辆的坐标补点
    :param savepath 文件保存路径
    :param savename 保存的文件名
    :param startend :待补点起终点坐标
    :return: 返回补点GPS坐标DataFrame
    """
    max_coordinate_num = 0
    Selected_num = []  # 存储被选中的车辆
    TempbackupTrunk = pd.DataFrame(columns=None)  # 存被选中车辆补点坐标
    Filleddf = pd.DataFrame(columns=None)   #存储最终的补点坐标
    if choose == 0:
        for trunk in trunkdict.keys():
            csvfile = trunk + ".csv"
            singleTrunkpath = os.path.join(Trunkpath, csvfile)
            try:

                df = pd.read_csv(singleTrunkpath, header=None, usecols=[2, 3])  # 读时间经纬度
                df = df.iloc[trunkdict[trunk][2][0]:trunkdict[trunk][2][1], :]
                rownum = df.shape[0]
                if (rownum > max_coordinate_num):
                    max_coordinate_num = rownum
                    backupTrunk = df
                    tem_num = trunk  #临时被选定车辆车牌号
            except Exception as e:
                print(e)
        if max_coordinate_num > 20:
            savefile = savename + ".csv"
            backupTrunk.to_csv(os.path.join(savepath, savefile), header=0, index=0)
            print("**********补点文件(不包含待补路段的起始点坐标)已保存到{},选定补点车辆为：{}**********".format(os.path.join(savepath, savefile),
                                                                                    Selected_num))
    elif choose == 1:
        pass
    else:
                ...
                if line.strip():
                    ...
                    if dic:
                        ...
                        for key in dic.keys():  #此循环为补充一个路段
                            coordinatenum = dic[key][2][1] - dic[key][2][0]
                            if coordinatenum > maxcoordinatenum:
                                maxcoordinatenum = coordinatenum
                                tem_num = key
                        csvfile = tem_num + ".csv"
                        ...
                        df = df.reset_index(drop=True) #重置索引,并删除之前的索引
                        df[2] = 2   #标记  2 表示补充点
                        df.loc[0] = [startend[0],startend[1],1]  #加入路段起始坐标  标记为1 代表起点终点坐标
                        df.loc[df.shape[0]] = [startend[2],startend[3],1]
                        AllFilledpoint = pd.concat([AllFilledpoint, df], ignore_index=True)
...
SaveFilename = 'text'
Filledtxt= 'H:\GPS_Data\\20170901\Top20\SimilarArea\old\\4b3142ee-5d31-416c-9653-fc3368e24055SimilarAreas.txt'
FillTracks(Trunkpath,Filledtxt,FilledSavePath,SaveFilename)
"""
...
"""
#寻找待补路段示例：
FindAllRoute('H:\GPS_Data\\20170901\Top20\\NOTimeText',
             'H:\GPS_Data\\20170901\Top20\Trajectory\\NotimeTra')
"""


#找出相近区域
filpa = "H:\GPS_Data\\20170901\Top20\Meshed"
savep = "H:\GPS_Data\\20170901\Top20\SimilarArea\\NoTimeSimilarArea"
paths = findtxtpath(r'H:\GPS_Data\\20170901\Top20\Trajectory\\NotimeTra')
#print(paths)
for pa in paths:
